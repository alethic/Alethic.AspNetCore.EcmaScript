...
public abstract class OutOfProcessNodeInstance : INodeInstance
{
	...
	const string ConnectionEstablishedMessage = "[Alethic.AspNetCore.EcmaScript.Node:Listening]";
	readonly TaskCompletionSource<object> _connectionIsReadySource = new TaskCompletionSource<object>();
	bool _disposed;
	readonly StringAsTempFile _entryPointScript;
	FileSystemWatcher _fileSystemWatcher;
	int _invocationTimeoutMilliseconds;
	bool _launchWithDebugging;
	readonly Process _nodeProcess;
	int? _nodeDebuggingPort;
	bool _nodeProcessNeedsRestart;
	readonly string[] _watchFileExtensions;
		...
		string nodePath)
	{
		...
		var startInfo = PrepareNodeProcessStartInfo(_entryPointScript.FileName, projectPath, commandLineArguments, environmentVars, _launchWithDebugging, debuggingPort, nodePath);
	...
	public async Task<T?> InvokeExportAsync<T>(CancellationToken cancellationToken, string moduleName, string exportNameOrNull, params object[] args)
	{
		...
		using (var combinedCancellationTokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, timeoutSource.Token))
		{
			...
			try
			{
				...
				return await InvokeExportAsync<T>(new NodeInvocationRequest
				{
	...
	/// </summary>
	/// <typeparam name="T">The JSON-serializable data type that the Node.js code will asynchronously return.</typeparam>
	/// <param name="request">Specifies the Node.js function to be invoked and arguments to be passed to it.</param>
	/// <param name="cancellationToken">A <see cref="CancellationToken"/> that can be used to cancel the invocation.</param>
	/// <returns>A <see cref="Task{TResult}"/> representing the completion of the RPC call.</returns>
	protected abstract Task<T?> InvokeExportAsync<T>(NodeInvocationRequest request, CancellationToken cancellationToken);
	...
	/// <returns></returns>
	protected virtual ProcessStartInfo PrepareNodeProcessStartInfo(string entryPointFilename, string projectPath, string commandLineArguments, IDictionary<string, string> environmentVars, bool launchWithDebugging, int debuggingPort, string nodePath)
	{
