...
using Alethic.AspNetCore.EcmaScript.SpaServices.StaticFiles;
...
public static class SpaStaticFilesExtensions
{
	...
	public static void AddSpaStaticFilesImproved(
		this IServiceCollection services,
		Action<StaticFiles.SpaStaticFilesOptions>? configuration = null)
	{
		services.AddSingleton<StaticFiles.ISpaStaticFileProvider>(serviceProvider =>
		{
			// Use the options configured in DI (or blank if none was configured)
			var optionsProvider = serviceProvider.GetService<IOptions<StaticFiles.SpaStaticFilesOptions>>()!;
			...
			{
				throw new InvalidOperationException($"No {nameof(StaticFiles.SpaStaticFilesOptions.RootPath)} " +
	...
	public static void UseSpaStaticFilesImproved(this IApplicationBuilder applicationBuilder)
	{
	...
	public static void UseSpaStaticFiles(this IApplicationBuilder applicationBuilder, StaticFileOptions options)
	{
		...
		applicationBuilder.UseSpaStaticFilesInternal(staticFileOptions: options,
	...
	{
		var spaStaticFilesService = app.ApplicationServices.GetService<StaticFiles.ISpaStaticFileProvider>();
		...
		else if (!allowFallbackOnServingWebRootFiles)
		{
			throw new InvalidOperationException($"""
				...
				first register an {nameof(StaticFiles.ISpaStaticFileProvider)} in the service provider, typically
				by calling services.{nameof(AddSpaStaticFilesImproved)}.
				""");
		}
		else
		{
